...
export class TuringProxy {
	...
	constructor(dllTuringPath: string, exeTuringPath: string = exe, nodeWinaxPath?: string) {
		this.execProcess = TuringProxy.createTuringClient(dllTuringPath, exeTuringPath, nodeWinaxPath);
	}

	public static createTuringClient(dllTuringPath: string, exeTuringPath: string = exe, nodeWinaxPath?: string): ChildProcess {
		const client = exec(`${exeTuringPath} --dll ${dllTuringPath}${nodeWinaxPath ? ` --winax ${nodeWinaxPath}` : ''}`)

		return client
	}

	public static execFunctionInTuringClient<T, R>(ep: ChildProcess, cb: FunctionInTuringClinet<T, R>, arg?: T): Promise<R> {
	...
	public exec<ResultType, ArgumentsType = void>(arg: ArgumentsType, cb: FunctionInTuringClinet<ArgumentsType, ResultType>) {
		return TuringProxy.execFunctionInTuringClient<ArgumentsType, ResultType>(this.execProcess, cb, arg)
	}
